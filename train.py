...
data_collator = DataCollatorForLanguageModeling(
    tokenizer, mlm=False
)  # fix some performance thing here
...
tokens = dataset["train"][6]["input_ids"]

print(tokenizer.decode(tokens))
print("len", len(tokens))

training_args = TrainingArguments(
    output_dir=f"models/{MODEL_NAME}-python-truncate-natural-prompt",
    ...
    report_to="wandb",
    save_total_limit=5,
    run_name=f"{MODEL_NAME}-python-truncate-natural-prompt",
)
